...
public class Calendrier extends JPanel implements ActionListener,ImagePlugins {
	...
	public Calendrier(){
		super();
		container=new JInternalFrame("JClock");
		this.x0  = 100;
		this.y0  = 100;
		this.mois  = 1;
		this.annee  = 1990;
		...
		new SLabel(string(this.mois)+" "+this.annee,1,14,150,0,120,30,this);
		...
		remplirVect();
		afficherGrille(this.mois,this.annee);
	}	
	public Calendrier(int x0,int y0,int iMois,int iAnnee,JInternalFrame container){
		super();
	...
	public Component actionOnPlugin(JInternalFrame jif, JDesktopPane desktop) {
		new Calendrier(100,100,02,1994,jif);
		return jif;
